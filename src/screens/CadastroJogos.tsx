...
function CadastroJogos(): React.JSX.Element {
  ...
  const checkUniqueName = async () => {
    try {
      const response = await axios.get(`http://10.137.11.207:8000/api/check/unique`, {
        params: {
          nome: nome
        }
      });
...
const cadastrarJogos = async () => {
  if (validateForm() && await checkUniqueName()) {
    ...
    try {
      ...
      const response = await axios.post('http://10.137.11.207:8000/api/register/games', data);
    ...
    <StatusBar barStyle="dark-content" backgroundColor={'#fff'}></StatusBar>
        ...
        <View style={styles.footer}>
                <TouchableOpacity onPress={() => navigation.navigate('Cadastro')}>
                    <Image 
                    source={require('../assets/imagem/noteSelect.png')}
