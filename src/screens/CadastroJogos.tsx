...
function CadastroJogos(): React.JSX.Element {
  ...
  const [nome, setNome] = useState<string>(""); // Campo nome
  const [preco, setPreco] = useState<string>(""); // Campo preço
  const [descricao, setDescricao] = useState<string>(""); // Campo descrição
  const [classificacao, setClassificacao] = useState<string>(""); // Campo classificação
  const [plataformas, setPlataformas] = useState<string>(""); // Campo plataformas
  const [desenvolvedor, setDesenvolvedor] = useState<string>(""); // Campo desenvolvedor
  const [distribuidora, setDistribuidora] = useState<string>(""); // Campo distribuidora
  const [categoria, setCategoria] = useState<string>(""); // Campo categoria
  const [errors, setErrors] = useState<string>(""); // Campo erros
  const [registeredNames, setRegisteredNames] = useState<string[]>([]); // Array de nomes registrados
  ...
  const validateForm = () => {
    ...
    } else if (registeredNames.includes(nome)) {
      newErrors.nome = "Este nome já foi registrado";
    }
    if (!preco) {
  ...
  const cadastrarJogos = async () => {
    if (validateForm()) {
      ...
      if (response.status === 200) {
        setRegisteredNames([...registeredNames, nome]); // Adiciona o nome ao array de nomes registrados
        resetFields();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Image style={styles.imagem} source={require('../assets/imagem/logo.png')}></Image>
      </View>
